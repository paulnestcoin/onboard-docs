...
4. **`Escrow`:** this is a 3-dimensional mapping that holds the record of assets currently locked in the SCEA and unavailable for spending. It is organized by token to make lookup faster. 
    ...
    - **Second-level map:** Under `OrderType`, maps either client addresses or order IDs to the locked value. For interactions with non-custodial wallets, the `ClientOrderKey` is a base 64 address.
