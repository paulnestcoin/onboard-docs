...
<Note>If the webhook endpoint does not specify a `200 0K` HTTP response code on receipt of webhook data, there will be subsequent attempts to resend it until the retry attempts are exhausted.<br/><br/>Number of retry attempts is currently set at 3.</Note>
...
| Event | Description |
| -------- | ------------------------------------- |
| order.state_transition | Triggered when an [order's state](#order-states) changes, such as from `INITIATED` to `DEPOSITED`. Includes detailed payload data for tracking. See sample below. |
...
<Tip>
A `terminal` state indicates that the state/status cannot be reversed, or transit to a different state.<br/><br/>It usually means the order has terminated, either as completed or cancelled.
</Tip>
